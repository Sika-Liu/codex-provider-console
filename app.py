...
document.head.insertAdjacentHTML('beforeend','<style>.provider-actions{display:flex;gap:6px;margin-left:auto;opacity:0;pointer-events:none;transition:opacity .15s}.provider-card:hover .provider-actions,.provider-card:focus-within .provider-actions{opacity:1;pointer-events:auto}.provider-action{min-width:52px;height:30px;padding:0 9px;border:1px solid #d7d9dd;border-radius:6px;background:#fff;color:#30343a;font:inherit;font-size:12px;cursor:pointer}.provider-action:hover{border-color:#8a929c;background:#f4f5f6}.provider-action.danger{color:#b42318}.provider-main{min-width:0}.provider-card.active .provider-action{background:#fff}@media(max-width:760px){.provider-card{align-items:flex-start;flex-wrap:wrap}.provider-actions{width:100%;margin-left:47px;opacity:1;pointer-events:auto}.provider-action{flex:1}}</style>');
...
function renderList(){const profiles=state.profiles;$('#list-count').textContent=`${profiles.length} 个供应商配置；可拖动排序，点击编辑进入详情`;$(' #provider-list'.trim()).innerHTML=profiles.length?profiles.map(p=>`<div class="provider-card ${p.id===state.active?'active':''}" onclick="openProvider('${esc(p.id)}')"><div class="handle">⋮⋮</div><div class="badge">${esc((p.name||p.id).slice(0,1).toUpperCase())}</div><div class="provider-main"><div class="card-name">${esc(p.name)} ${p.id===state.active?'<span class="section-hint">使用中</span>':''}</div><div class="card-meta">${p.auth_mode==='chatgpt'?'官方登录':'纯 API'} · ${p.wire_api==='responses'?'Responses API':'Chat Completions'} · ${esc(p.base_url||'不写入 API 文件')}</div></div><div class="provider-actions"><button class="provider-action" type="button" title="使用此供应商" onclick="event.stopPropagation();activateFromList('${esc(p.id)}')" ${p.id===state.active?'disabled':''}>使用</button><button class="provider-action" type="button" title="编辑供应商" onclick="event.stopPropagation();openProvider('${esc(p.id)}')">编辑</button><button class="provider-action" type="button" title="复制供应商" onclick="event.stopPropagation();copyProvider('${esc(p.id)}')">复制</button><button class="provider-action danger" type="button" title="删除供应商" onclick="event.stopPropagation();deleteProvider('${esc(p.id)}')">删除</button></div></div>`).join(''):'<div class="empty">尚未添加供应商。可先添加 API 供应商或官方登录档案。</div>'}
async function activateFromList(id){try{const result=await api(`/api/providers/${encodeURIComponent(id)}/activate`,{method:'POST'});state.active=id;setManagedRestartAvailable(true);note(`已使用 ${id}，备份编号：${result.backup_id}。现在可重启 Codex 读取新配置。`,'list-notice');await refreshAll()}catch(e){note(e.message,'list-notice')}}
function copyProvider(id){const source=state.profiles.find(p=>p.id===id);if(!source)return;let suffix=1,candidate=`${id}-copy`;while(state.profiles.some(p=>p.id===candidate)){suffix+=1;candidate=`${id}-copy-${suffix}`}state.current={...structuredClone(source),id:candidate,name:`${source.name} copy`};openDetail();note('已创建副本，请确认后保存。官方登录副本需要重新登录。')}
