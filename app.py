...
document.head.insertAdjacentHTML('beforeend','<style>.detail-top .provider-back{display:inline-flex;align-items:center;gap:7px;height:34px;padding:0 13px;border:1px solid #d7dce2;border-radius:999px;background:#fff;color:#313840;font:inherit;font-size:13px;font-weight:650;cursor:pointer;white-space:nowrap;transition:background .15s,border-color .15s,color .15s}.detail-top .provider-back:hover{background:#f2f5f7;border-color:#9da6b0;color:#111827}.detail-top .provider-back:focus-visible{outline:2px solid #1683ff;outline-offset:2px}.detail-top .provider-back .back-arrow{font-size:17px;line-height:1}</style>');
const detailBack=document.querySelector('.detail-top .back');if(detailBack){detailBack.className='provider-back';detailBack.type='button';detailBack.title='返回供应商列表';detailBack.innerHTML='<span class="back-arrow" aria-hidden="true">←</span><span>返回列表</span>'}
