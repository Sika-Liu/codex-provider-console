...
function updatePreview(){if(!state.current)return;const p=gather();if(p.auth_mode==='apikey'){if(!$('#auth-preview').value.trim())$('#auth-preview').value=JSON.stringify({OPENAI_API_KEY:$('#p-key').value},null,2)}else $('#auth-preview').value=officialAuthStatus(state.current)}
