...
document.head.insertAdjacentHTML('beforeend','<style>#list-view.console-content{max-width:none;margin:22px 0;padding:0 26px}.provider-actions{display:flex;gap:6px;margin-left:auto;opacity:0;pointer-events:none;transition:opacity .15s}.provider-card{cursor:default}.provider-card:hover .provider-actions,.provider-card:focus-within .provider-actions{opacity:1;pointer-events:auto}.provider-action{min-width:52px;height:30px;padding:0 9px;border:1px solid #d7d9dd;border-radius:6px;background:#fff;color:#30343a;font:inherit;font-size:12px;cursor:pointer}.provider-action:hover{border-color:#8a929c;background:#f4f5f6}.provider-action.danger{color:#b42318}.provider-main{min-width:0}.provider-card.active .provider-action{background:#fff}@media(max-width:760px){#list-view.console-content{padding:0 13px}.provider-card{align-items:flex-start;flex-wrap:wrap}.provider-actions{width:100%;margin-left:47px;opacity:1;pointer-events:auto}.provider-action{flex:1}}</style>');
...
function renderList(){const profiles=state.profiles;$('#list-count').textContent=`${profiles.length} 个供应商配置；点击编辑按钮进入详情`;$(' #provider-list'.trim()).innerHTML=profiles.length?profiles.map(p=>`<div class="provider-card ${p.id===state.active?'active':''}"><div class="handle">⋮⋮</div><div class="badge">${esc((p.name||p.id).slice(0,1).toUpperCase())}</div><div class="provider-main"><div class="card-name">${esc(p.name)} ${p.id===state.active?'<span class="section-hint">使用中</span>':''}</div><div class="card-meta">${p.auth_mode==='chatgpt'?'官方登录':'纯 API'} · ${p.wire_api==='responses'?'Responses API':'Chat Completions'} · ${esc(p.base_url||'不写入 API 文件')}</div></div><div class="provider-actions"><button class="provider-action" type="button" title="使用此供应商" onclick="activateFromList('${esc(p.id)}')" ${p.id===state.active?'disabled':''}>使用</button><button class="provider-action" type="button" title="编辑供应商" onclick="openProvider('${esc(p.id)}')">编辑</button><button class="provider-action danger" type="button" title="删除供应商" onclick="deleteProvider('${esc(p.id)}')">删除</button></div></div>`).join(''):'<div class="empty">尚未添加供应商。可先添加 API 供应商或官方登录档案。</div>'}
