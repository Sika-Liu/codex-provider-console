...
def wait_for_session_archive_state(thread_id: str, archived: bool, timeout_seconds: float = 5.0) -> None:
    """Wait until App Server storage and the panel listing agree on archive state."""
    deadline = time.monotonic() + timeout_seconds
    while True:
        thread_states = app_server_thread_states()
        sessions = list_archived_server_sessions() if archived else list_server_sessions()
        listed = any(item["id"] == thread_id for item in sessions)
        registered = thread_states is not None and thread_states.get(thread_id) is archived
        if registered and listed:
            return
        if time.monotonic() >= deadline:
            action = "归档" if archived else "取消归档"
            raise RuntimeError(f"Codex 已接受{action}命令，但服务器会话状态尚未完成同步")
        time.sleep(0.2)
...
async function testCurrent(){if(providerDiagnosticPoll)return;try{renderProviderDiagnosticJob({progress:2,stage:'正在准备供应商诊断',elapsed_ms:0});const started=await api('/api/providers/diagnose-progress',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(gather())});const poll=async()=>{try{const job=await api(`/api/provider-diagnostics/${encodeURIComponent(started.id)}`);if(job.status==='running'){renderProviderDiagnosticJob(job);providerDiagnosticPoll=setTimeout(poll,400);return}providerDiagnosticPoll=null;if(job.status==='completed')showDoctor(job.result);else{renderProviderDiagnosticJob({progress:100,stage:'供应商诊断失败',elapsed_ms:job.elapsed_ms||0});$('#doctor-checks').innerHTML=`<div class="doctor-check fail"><b>诊断错误</b><small>${esc(job.detail||'诊断任务失败')}</small></div>`;$('#doctor-close').style.display=''}}catch(error){providerDiagnosticPoll=null;renderProviderDiagnosticJob({progress:100,stage:'供应商诊断失败',elapsed_ms:0});$('#doctor-checks').innerHTML=`<div class="doctor-check fail"><b>诊断错误</b><small>${esc(error.message)}</small></div>`;$('#doctor-close').style.display=''}};providerDiagnosticPoll=setTimeout(poll,100)}catch(error){providerDiagnosticPoll=null;$('#doctor-summary').textContent='诊断请求未能启动。';$('#doctor-checks').innerHTML=`<div class="doctor-check fail"><b>诊断错误</b><small>${esc(error.message)}</small></div>`;$('#doctor-close').style.display='';$('#doctor-mask').classList.add('show')}}
 ...
 function showProxyForm(){proxyEditing=true;$('#proxy-success-view').hidden=true;$('#proxy-form-view').hidden=false;$('#proxy-certificate').value='';$('#proxy-private-key').value='';updateProxyConfig()}
 ...
 async function refreshReverseProxyStatus(){const status=$('#proxy-status');if(!status)return;status.textContent='正在读取状态…';status.classList.remove('running');try{const result=await api('/api/reverse-proxy/status');if(result.domain)$('#proxy-domain').value=result.domain;if(result.upstream)$('#proxy-upstream').value=result.upstream;updateProxyConfig();status.textContent=result.running?`运行中 · ${result.domain||'已部署'}`:result.configured?'已配置，当前已停用':'尚未部署';status.classList.toggle('running',result.running);if(result.configured&&!proxyEditing)showProxySuccess(result);else if(!result.configured)showProxyForm()}catch(error){status.textContent='状态读取失败';showProxyForm()}}
 ...
 document.getElementById('proxy-apply')?.addEventListener('click',applyReverseProxy);document.getElementById('proxy-edit')?.addEventListener('click',showProxyForm);document.getElementById('proxy-disable')?.addEventListener('click',disableReverseProxy);
